...
class Entitlement(Model):
    ...
    name = UnicodeAttribute()
    description = UnicodeAttribute(null=True)
    action_type = ListAttribute(of=ActionTypeEnumAttribute)
    object_type = ListAttribute(of=ObjectTypeEnumAttribute)
...
class User(Model):
    ...
    entitlements = MapAttribute(attr_name="entitlements", default={})
    ...
    def add_entitlement(self, entitlement):
        ...
        entitlement_map = entitlement.attribute_values
        entitlement_map["id"] = str(entitlement_map["id"])
        for str_enum in ["action_type", "object_type"]:
            entitlement_map[str_enum] = [x.value for x in entitlement_map[str_enum]]

        self.entitlements.attribute_values[entitlement.id] = entitlement_map
        self.save()
